<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=00ff00&height=300&section=header&text=Security%20Analyst%20and%20Pentester&fontSize=45&animation=fadeIn&fontAlignY=38&desc=Breaching%20Defenses%20-%20Hunting%20Threats&descAlignY=55&descAlign=62" width="100%"/>
</div>

### 👋 Hi there, I'm a Purple Team Enthusiast!

I bridge the gap between **Offensive Security** and **Defensive Operations**. 

My core focus involves uncovering critical vulnerabilities (Red Team) and validating them, while simultaneously learning how to detect and respond to these threats using modern SOC technologies (Blue Team).

---

### 🛡️ Vulnerability Management & Pentesting
I have extensive experience in **Penetration Testing** and **Vulnerability Validation** (Triage). I verify findings from external researchers to ensure validity and impact.

**🚀 Bugs I've Successfully Exploited & Validated:**
* **IDOR** (Insecure Direct Object References)
* **SQL Injection** (SQLi)
* **Cross-Site Scripting** (XSS)
* **Path Traversal** / LFI
* Broken Access Control & Logic Flaws

---

### 🔭 Current Focus & Learning Journey
I am expanding my arsenal into **Threat Intelligence** and **Endpoint Security**:
* **SIEM:** Monitoring and Log Analysis with **Wazuh**.
* **Threat Intel:** Managing indicators of compromise (IoC) with **OpenCTI**.
* **EDR:** Exploring Endpoint Detection & Response mechanisms.

---

### 🏆 TryHackMe Stats
<a href="https://tryhackme.com/p/lealore">
  <img src="https://tryhackme-badges.s3.amazonaws.com/lealore.png" alt="TryHackMe Badge"/>
</a>

---

### 🛠️ The Arsenal
Tools & Technologies I use for attacking and defending:

<p align="left">
  <img src="https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black" />
  <img src="https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white" />
  <img src="https://img.shields.io/badge/Burp%20Suite-FF6633?style=for-the-badge&logo=burpsuite&logoColor=white" />
  <img src="https://img.shields.io/badge/Postman-FF6C37?style=for-the-badge&logo=postman&logoColor=white" />
  <img src="https://img.shields.io/badge/Wazuh-00B5E2?style=for-the-badge&logo=wazuh&logoColor=white" />
  <img src="https://img.shields.io/badge/Threat_Intel-OpenCTI-001F3F?style=for-the-badge&logo=security&logoColor=white" />
</p>
